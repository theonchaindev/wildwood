...
// Play-to-earn conversion: 1 acorn = 1 $WILD (SPL token, 6 decimals),
...
export async function POST(req: Request) {
  ...
  if (!secret || !mintAddr) {
    return NextResponse.json({
      status: "pending",
      acorns,
      message: "Conversion queued — the $WILD mint isn't configured on this server yet",
    });
  }
  ...
  try {
    ...
    // the player's $WILD account (created for them on first conversion)
