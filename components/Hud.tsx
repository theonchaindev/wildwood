...
function PlayerOffers() {
  ...
  return (
    <>
      ...
      <div className="shop-section">🍃 Convert to $WILD — straight to your wallet</div>
      {s.account.wallet ? (
        <>
          <div className="shop-note" style={{ marginTop: 0 }}>
            1 🍃 = 1 $WILD, minted on Solana to{" "}
            <b>{s.account.wallet.slice(0, 4)}…{s.account.wallet.slice(-4)}</b>
            {" "}· devnet beta — switch Phantom to devnet to see your tokens
          </div>
          <div className="wallet-balance">
            👛 In your wallet: <b>{walletWood === null ? "…" : walletWood.toLocaleString()} $WILD</b>
          </div>
        ...
        <div className="shop-note" style={{ marginTop: 0 }}>
          Sign in with Phantom on the intro screen to convert WILD into $WILD.
        </div>
      )}
...
function GiftsSection() {
  ...
  const send = async () => {
    ...
    if (tip > 0) {
      if (s.acorns < tip) {
        setErr("Not enough WILD for that tip");
        return;
      }
      payload.acorns = tip;
    }
      ...
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        {data?.players.map((p, i) => (
          <div key={p.name} className={`lb-line ${p.name === myName ? "me" : ""}`}>
            ...
            <span className="lb-stats">Lv {p.level} · {p.wood.toLocaleString()} $WILD</span>
          </div>
        ))}
        <button className="btn block" onClick={onClose}>Close</button>
      </div>
  ...
  return (
    <div className="modal-backdrop" onClick={close}>
      <div className="note-paper" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="note-section">🍃 The $WILD coin</div>
        {data === null ? (
        ...
        <div className="note-line small">1 🍃 = 1 $WILD · convert at The Vault</div>

        <div className="note-section">💸 Paid out to survivors so far</div>
        <div className="note-big">
          {data ? data.totalAcorns.toLocaleString() : "…"} $WILD
        </div>
